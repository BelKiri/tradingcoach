...
- Coaching sessions now record LLM response latency in the database for operational review.
...
- Redesigned first-session AI analysis with three-section narrative and independently-ranked action plan.
...
- Trading account creation is now limited to 3 accounts per user during the beta. Contact the operator for expanded access.
...
- Upload endpoint rejects files larger than 10 MB to prevent backend memory pressure.
